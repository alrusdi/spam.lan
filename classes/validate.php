...
class validate extends ArrayObject {
	...
	public static function min_max_length($value, $min_length, $max_length)
	{
	...
	public function rules_str($field, $rules)
	{
		$rules=explode(';', $rules);
		foreach ($rules as $val) {
			$params=Array();
			$params_pos=mb_strpos($val, '[');
			if ( $params_pos>0 ) {
				$ipars=trim(mb_substr($val, $params_pos+1, -1));
				$params=explode(',',$ipars);
				$val=mb_substr($val, 0, $params_pos);
			}
			$this->rule($field, $val, $params);
		}
		return $this;
	}	
	...
	public function check($allow_empty = FALSE)
	{
		...
		{
			$iv=null;
			if ( $params_pos=mb_strpos($field, '[') ) {
				$keys=explode(']', str_replace('[', '', mb_substr($field, $params_pos)));
				$keys=array_slice($keys, -1);
				$iv=self::get_inner_value($keys, $this);
			}
	...
	public function errors($file = NULL, $translate = FALSE)
	{
		...
		foreach ($this->_errors as $field => $set)
		{
			...
			if (is_array($values[':value']))
			{
				...
				$values[':value'] = implode(', ', self::flatten($values[':value']));
			}
			...
			if ($params)
			{
				foreach ($params as $key => $value)
				{
					...
					$iparams[':param'.($key + 1)] = $value;
				}
			}
			$message = $this->get_message($error, $field, $iparams);
	...
	private function get_message($error, $field, $params){
		$messages=array(
			...
			'min_max_length'=> ':field must be :param1 to :param2 characters long',
		...
		$message = $field.' field is incorrect';
		if ( !empty($messages[$error]) ) {
			$message=$messages[$error];
			$message=str_replace(':field', $field.' field', $message);
	...
	private static function get_inner_value($keys, $from) {
		foreach ($keys as $key) {
			if ( isset($from[$key]) ) {
				$from=$from[$key];
			} else {
